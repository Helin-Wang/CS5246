...
**实现**：`src/location_extractor.py`，入口 `extract_location(text, title) → LocationResult`。全部从文本抽取，不依赖 GKG V1LOCATIONS。

**4 步流程**：

1. **文本坐标 regex**：扫描全文匹配 `23.5°N 121.6°E`、`latitude 38.1, longitude 142.4` 等显式坐标 → 命中则 `confidence="coords_text"`，否则继续。

2. **NER 提取候选**：spaCy `en_core_web_sm` 在 title + 正文前 3 句提取 GPE/LOC；ORG 仅在 alias 字典中时接受。优先级：与灾难触发词（struck/hit/flooded/burned/made landfall）同句的实体 > 其他 > body > title。过滤 dateline（"CITY (Reuters)" 格式首实体跳过）。

3. **国家优先解析**：对每个候选**先查国家索引，再查城市索引**（防城市同名遮蔽国家，如 "India" 误解析为塞尔维亚某城市）。
   - 国家索引：alias 字典（US 50州、加拿大13省、澳大利亚各州、印度各邦、中国省份、日本都道府县、印尼岛屿、河流防误匹配、海外领土）+ pycountry + geonamescache
   - 城市索引：geonamescache ~25k 城市，同名取人口最多者；过滤长度 <3 或纯数字（防邮编误匹配）

4. **Fallback**：候选均无法解析时保留 `location_text=gpes[0]`，`country_iso2=None`；尝试 `"City, Country"` partial match。
...
class LocationResult:
    location_text: Optional[str] = None   # 地名
    country_iso2:  Optional[str] = None   # ISO-2 国家代码
    lat:           Optional[float] = None # 仅文本坐标 regex 结果，否则 None
    lon:           Optional[float] = None
    confidence:    str = "none"           # "coords_text" | "location" | "none"
```

`lat/lon` **只来自文本中显式坐标**，不推断城市质心——推断坐标偏差可达数百公里，对 Module D 聚类有害。

#### 4.3.3 时间抽取

**实现**：待实现于 `src/time_extractor.py`，策略：
...
4. DR 特殊：提取 "since [month]" 起点，精度为月

**GT 标注**（LLM 自动，`scripts/label_times.py`）：

| 字段 | 含义 |
|------|------|
| `event_date_raw` | 文中原始时间表达（如 "Thursday", "since May"） |
| `event_date_iso` | LLM 推断日期（YYYY-MM-DD），以 article timestamp 解析相对表达 |
| `granularity` | `day` / `month` / `year` / `unknown` |
| `time_type` | `event_date` / `date_range` / `duration_only` / `unknown` |

- 输入：`data/llm_labels/time_labels_test_input.csv`（778行，过滤 not_related）
- 输出：`data/llm_labels/time_labels_test.csv`（服务器 Job 556958 标注中）

#### 4.3.4 数值参数（regex + 单位归一化）
...
**待实现**（当前 `unified_event_extractor.py` 缺失）：
